...
class CompGraph(DiGraph):
    ...
    def set_colocation_group(self, node_id, colocation_group):
        if node_id not in self.nodes:
            raise ValueError("node {0} does not exist".format(node_id))
        if "colocation_group" in self.nodes[node_id]:
            raise ValueError("colocation_group already exists")
...
class DeviceGraph(DiGraph):
    ...
    def generata_fully_connected_topo(self, device_number, intra_node_band, inter_node_band, max_num_device_per_node):
        ...
        def get_device_memory() -> int :
            try:
                ...
                first_gpu_mem_mb = int(output.strip().split('\n')[0])
                return first_gpu_mem_mb * (1024**2)  # MB -> byte
            except Exception as e:
                raise ValueError("not GPU found")

        mem = get_device_memory()
        ...
        for node_index in range(num_nodes):
            ...
            for device_id in range(start_device_id, end_device_id):
                ...
                self.add_new_node(device_id_name, mem)
