...
class Task:
    def __init__(self, name, device, compute_cost):
        self.name = name
        self.device = device
...
def evaluate_mcmc(G: CompGraph, device_ops_mapping: Mapping, compute_costs, comm_costs):
    devices = device_ops_mapping.keys()
    # Build Task objects and assign devices
    tasks = {}
    for op in compute_costs:
        for d, ops in device_ops_mapping.items():
            if op in ops:
                tasks[op] = Task(op, d, compute_costs[op])
                break
